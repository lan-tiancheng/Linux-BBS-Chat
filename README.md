...
Qt 前端包含独立登录/注册窗口、聊天页面、BBS 卡片页面、浅绿色 QSS 美化、历史消息、离线私聊保存、发帖/回复附件上传，以及点击附件链接下载到 `downloads/`。详细说明见 [`README_QT_INTEGRATION.md`](README_QT_INTEGRATION.md)。
...
需要先登录。`GROUP` 会发送给所有在线用户，包括发送者；`PRIVATE` 会检查目标用户是否已注册，目标离线时会保存到 `logs/chat.log`，之后可通过 `HISTORY` 查看。
